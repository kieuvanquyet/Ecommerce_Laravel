(function (global, factory) {
    ...
    function getContext(selector, context = document) {
        return (isString(selector) &&
            parseSelector(selector).isContextSelector) ||
            isDocument(context)
            ? context
    ...
    const parseSelector = memoize((selector) => {
        ...
        const selectors = [];
        for (let sel of (_a = selector.match(splitSelectorRe)) != null
            ? _a
            : []) {
    ...
    const parsePositionSelector = memoize((selector) => {
        ...
        return ~index2
            ? [selector.slice(0, index2), selector.slice(index2 + 1)]
            : [selector, ""];
    });
    ...
    const dirs$1 = { width: ["left", "right"], height: ["top", "bottom"] };
    ...
    function getViewportHeight() {
        ...
        if (!vhEl) {
            vhEl = $("<div>");
            css(vhEl, { height: "100vh", position: "fixed" });
    ...
    var Margin = {
        ...
        observe: [
            mutation({ options: { childList: true } }),
            mutation({
                options: { attributes: true, attributeFilter: ["style"] },
                target: ({ $el }) => [$el, ...children($el)],
            }),
            resize({ target: ({ $el }) => [$el, ...children($el)] }),
        ],
        update: {
            read() {
                return { rows: getRows(children(this.$el)) };
            },
            write({ rows }) {
    ...
    var SliderAutoplay = {
        ...
        data: { autoplay: false, autoplayInterval: 7e3, pauseOnHover: true },
        connected() {
    ...
    var SliderDrag = {
        ...
        data: { draggable: true, threshold: 10 },
        created() {
    ...
    function registerEvent(instance, event, key) {
        ...
        if (
            ...
            (filter && !filter.call(instance, instance))
        ) {
            return;
        }
    ...
    function initPropsObserver(instance) {
        ...
        observer.observe(el, { attributes: true, attributeFilter: filter });
        registerObserver(instance, observer);
    }
    ...
    var lightbox = {
        ...
        props: { toggle: String },
        data: { toggle: "a" },
        computed: { toggles: ({ toggle }, $el) => $$(toggle, $el) },
        watch: {
            toggles(toggles) {
                this.hide();
                for (const toggle of toggles) {
    ...
    function transformFn(prop, el, stops) {
        ...
        return (css2, percent) => {
            css2.transform = `${css2.transform || ""} ${prop}(${getValue(
                stops,
                percent
            )}${unit})`;
        };
    ...
    function backgroundCoverFn(prop, el, stops, props2) {
        ...
        const dimEl = { width: el.offsetWidth, height: el.offsetHeight };
        const bgProps = ["bgx", "bgy"].filter((prop2) => prop2 in props2);
        const positions = {};
        for (const prop2 of bgProps) {
    ...
    function setBackgroundPosFn(bgProps, positions, props2) {
        return function (css2, percent) {
            for (const prop of bgProps) {
                ...
                css2[
                    `background-position-${prop.slice(-1)}`
    ...
    var parallax = {
        ...
        data: { target: false, viewport: 1, easing: 1, start: 0, end: 0 },
        computed: {
            target: ({ target }, $el) =>
    ...
    var slider = {
        ...
        observe: resize({ target: ({ slides, $el }) => [$el, ...slides] }),
        update: {
            write() {
                for (const el of this.navItems) {
    ...
    var sortable = {
        ...
        events: { name: pointerDown$1, passive: false, handler: "init" },
        computed: {
        ...
        update: {
            write(data) {
                ...
                css(this.drag, { top: y - offsetTop, left: x - offsetLeft });
    ...
    function parseProps(options) {
        ...
        return ["delay", "title"].reduce(
            ...
            { ...parseOptions(data(el, id), ["title"]), ...data$1 }
        );
    }
    var upload = {
        mixins: [I18n],
    ...
    var alert = {
        ...
        data: { animation: true, selClose: ".uk-alert-close", duration: 150 },
        events: {
            name: "click",
    ...
    var Video = {
        ...
        observe: [
            intersection({
                filter: ({ $el, autoplay }) =>
                    autoplay && autoplay !== "hover" && isVideo($el),
    ...
    var drop = {
        ...
        events: [
            ...
            { name: "beforehide", self: true, handler: "clearTimers" },
            {
                name: "hide",
                handler({ target }) {
                    if (this.$el !== target) {
    ...
    var heightMatch = {
        ...
        computed: { elements: ({ target }, $el) => $$(target, $el) },
        observe: resize({
            target: ({ $el, elements }) =>
                elements.reduce(
                ...
                if (this.expand) {
                    ...
                    if (this.offsetTop) {
                        if (isScrollingElement) {
                            ...
                            minHeight += ` - ${boxModelAdjust(
                                scrollElement,
                                "height",
                                css(scrollElement, "boxSizing")
                            )}px`;
                        }
                    }
                    if (this.offsetBottom === true) {
                        ...
                        minHeight += ` - ${
                            dimensions$1(query(this.offsetBottom, this.$el))
                                .height
                        }px`;
                    }
                }
    ...
    const IconComponent = {
        args: false,
        extends: Icon,
        data: (vm) => ({ icon: hyphenate(vm.constructor.options.name) }),
        beforeConnect() {
            addClass(this.$el, this.$options.id);
        },
    };
    ...
    const Marker = { extends: ButtonComponent, i18n: { label: "Open" } };
    const Totop = { extends: ButtonComponent, i18n: { label: "Back to top" } };
    ...
    var inverse = {
        ...
        data: { target: false, selActive: false },
        computed: {
    ...
    var leader = {
        ...
        update: {
            read() {
                ...
                return { width, fill: this.fill, hide: !this.matchMedia };
            },
    ...
    function install({ modal }) {
        ...
        modal.i18n = { ok: "Ok", cancel: "Cancel" };
        ...
        update: [
            {
                read() {
                    const targets = this.links.map(
                        (el) => getTargetedElement(el) || el.ownerDocument
                    );
    ...
    var toggle = {
        ...
        data: { href: false, target: false, mode: "click", queued: true },
        computed: {
            target: {
                get: ({ target }, $el) => {
